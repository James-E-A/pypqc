...
def main():
    ...
    # 1. Arbitrary data blob (e.g. your symmetric encryption key)
    k = urandom(69)

    # 2. Sender performs the kem-trans algorithm
    ek = kem_trans(kemalg, k, pk)

    # 3. Recipient performs the kem-trans unwrap algorithm
...
def kem_trans_unwrap(kemalg, ek, sk, *, wrapalg=aes_siv, kdf=shake256_256):
    ...
    wk, ct = ek[:-kemalg.lib.CRYPTO_CIPHERTEXTBYTES], ek[-kemalg.lib.CRYPTO_CIPHERTEXTBYTES:]
    try:
